...
def noise_inject(x: bytes) -> bytes:
    index_ = random.randint(0, len(x) - 1)
    return (
        x[:index_] + random.randbytes(random.randbytes(1)[0]) + x[index_ + 1 : len(x)]
    )

def rotation(x: bytes) -> bytes:
    ratio = random.randint(0,len(x)-1)
    return x[-ratio:]+x[:len(x)-ratio] 


def mix(x1: bytes,x2: bytes) -> bytes:
    pass
